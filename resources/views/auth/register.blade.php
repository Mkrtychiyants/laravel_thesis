...
<main class="w-3/5 mx-auto text-md text-black ">
        <section >
            ...
            <div class=" flex flex-col items-center  bg-white p-6  mx-auto">
                    <form  method="post" action="{{route('register_attempt')}}"  class ="flex w-4/5 flex-col justify-center gap-1 py-2" >
                            @csrf
                            <label for="user_name">Имя:</label>
                            <input class=" w-full text-lg border text-slate-300 border-stone-300 rounded-sm  inline-block border-2 border-stone-300 p-4 px-2 py-1 my-1 mr-1 @error('name') border-red-500 @enderror" name="user_name" id="user_name" type="text" placeholder="Иван">
                                @error('name')
